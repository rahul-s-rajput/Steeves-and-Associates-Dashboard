...
export default function EnrollmentDashboard() {
  ...
  const enrollmentKPIs = useMemo(() => {
    ...
    if (!processedEnrollmentData || processedEnrollmentData.length === 0) {
      return {
        total: { value: 0, change: 0, label: "Total Students", metric: "students" },
        ...
        indigenous: { value: 0, change: 0, label: "Indigenous Students", metric: "students" }
      };
    }
    ...
    const currentTotalStudents = currentYearData.reduce((sum, item) => sum + item.total_enrollment_headcount, 0);
    const currentDomesticStudents = currentYearData.reduce((sum, item) => sum + item.domestic_students_headcount, 0);
    const currentIntlStudents = currentYearData.reduce((sum, item) => sum + item.international_students_headcount, 0);
    const currentIndigenousStudents = currentYearData.reduce((sum, item) => sum + (item.indigenous_students_headcount || 0), 0);
    
    // Calculate previous year metrics for comparison
    const previousTotalStudents = previousYearData.reduce((sum, item) => sum + item.total_enrollment_headcount, 0);
    const previousDomesticStudents = previousYearData.reduce((sum, item) => sum + item.domestic_students_headcount, 0);
    const previousIntlStudents = previousYearData.reduce((sum, item) => sum + item.international_students_headcount, 0);
    const previousIndigenousStudents = previousYearData.reduce((sum, item) => sum + (item.indigenous_students_headcount || 0), 0);
    ...
    const indigenousStudentsChange = calculatePercentChange(currentIndigenousStudents, previousIndigenousStudents);
    ...
    return {
      ...
      indigenous: {
        value: currentIndigenousStudents,
        change: indigenousStudentsChange,
  ...
  const prepareEnrollmentData = (groupBy: 'university' | 'year') => {
    ...
    filteredEnrollmentData.forEach((item) => {
      ...
      // Accumulate enrollment components
  ...
  const prepareEnrollmentTrendData = () => {
    ...
    filteredEnrollmentData.forEach((item) => {
      ...
      // Accumulate values
      groupedData[uniName][year].domestic += item.domestic_students_headcount || 0;
      groupedData[uniName][year].international += item.international_students_headcount || 0;
  ...
  return (
    ...
    >
      ...
      {/* Enrollment Trends Chart - Priority visualization at the top */}
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Enrollment Trends</CardTitle>
            <CardDescription>Historical enrollment data showing domestic vs. international student numbers</CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Legend />
                {
                  // Create lines for each university showing domestic and international students
                  [...new Set(prepareEnrollmentTrendData().map(item => item.university))].map((uni, index) => {
                    ...
                    return [
                      ...
                      <Line 
                        ...
                        name={`${uni} - International`} 
                        stroke={COLORS[(colorIndex + 3) % COLORS.length]}
      ...
      {/* Enrollment Distribution - Second priority visualization */}
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
          ...
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Bar dataKey="Indigenous Students" stackId="a" fill="#FFBB28" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      {/* Completion Rates - Third priority visualization */}
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Completion Rates</CardTitle>
            <CardDescription>Program completion rates by degree level</CardDescription>
          </div>
          <div className="flex items-center space-x-2">
            <span className="text-sm text-muted-foreground">Group by:</span>
      ...
      {/* Domestic vs International Ratio - Fourth priority visualization */}
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Domestic vs. International Student Ratio</CardTitle>
          <CardDescription>Comparison of domestic and international student percentages (latest data)</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {/* Total Enrollment Comparison - Fifth priority visualization */}
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Total Enrollment Comparison</CardTitle>
          <CardDescription>Compare total student enrollment between universities (latest data)</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
